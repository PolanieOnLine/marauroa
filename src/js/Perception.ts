...
/**
 * The PerceptionListener interface provides methods that are called while
...
export class PerceptionListener {
	 ...
	 * @return true to stop further processing
	 */
	onModifiedAdded(_object: object, _changes: object) {
		return false;
	}
	...
	deleteChanges(object: any, diff: any) {
		...
		if (typeof(diff["s"]) !== "undefined") {
			for (var i in diff["s"]) {
				...
				// delete slot?
				if (MarauroaUtils.isEmpty(diff["s"][i])) {
					object.del(diff["s"][i]);
				} else {
		...
		if (typeof(diff["m"]) !== "undefined") {
			for (var i in diff["m"]) {
				...
				if (MarauroaUtils.isEmpty(diff["m"][i]["a"])) {
